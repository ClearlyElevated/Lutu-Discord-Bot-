...
class Forget extends Command {
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    if (!user) return reply("<a:aRedTick:584866618039861249> You haven't specified a valid user.");
    ...
    Infractions.findOne({
      ...
      if (message.guild.settings.moderationLogChannel !== "") {
        const Logger2 = new logHandler({ client: this.client, case: "pardon", guild: message.guild.id, member: user.user, reason: reason, moderator: message.author, amount: warnsToR, modOption: true });
