    ...
    if (mode === "on") {
      Lockdown.findOne({
        ...
        if (!g) {
          ...
          return reply("<a:aGreenTick:584866617780076576> This server is now under lockdown. All new messages will be deleted. Use `unlockdown [reason]` to unlock it.");
        }

        if (g.lockdownMode === "on") return reply("<a:aRedTick:584866618039861249> This server is already under lockdown. Use `unlockdown [reason]` to unlock it.");
        ...
        try {
          ...
          reply("<a:aRedTick:584866618039861249> I don't have permissions to geenrate a secured channel.");
        }
        ...
        reply("<a:aGreenTick:584866617780076576> This server is now under lockdown. All new messages will be deleted. Use `unlockdown [reason]` to unlock it.");
      });
    } else if (mode === "off") {
      Lockdown.findOne({
        guildID: message.guild.id
        ...
        if (!g) {
          ...
          try {
            ...
            reply("<a:aRedTick:584866618039861249> , failed to delete 'safe' channel. Probably missing permissions.");
          }
          return reply("<a:aGreenTick:584866617780076576> Lockdown mode has been disabled. Messages will no longer be deleted. Use `lockdown [reason]` to lock the server again.");
        }

        if (g.lockdownMode === "off") return reply("<a:aRedTick:584866618039861249> Server is not under lockdown. Use `lockdown [reason]` to lock it.");
        ...
        reply("<a:aGreenTick:584866617780076576> Lockdown mode has been disabled. Messages will no longer be deleted. Use `lockdown [reason]` to lock the server again.");
      });
    }
  }
}
