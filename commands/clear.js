...
class Clear extends Command {
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    if (!amount) return reply("<a:aRedTick:584866618039861249> Please specify an amount of messages to delete.");
    if (amount > 99) return reply("<a:aRedTick:584866618039861249> The number must be smaller than 100.");
    if (amount < 2) return reply("<a:aRedTick:584866618039861249> The number must be bigger than 1.");
    if (!amount && !user) return reply("<a:aRedTick:584866618039861249> Please mention a user or specify an amount of messages to delete!");
    amount = amount + 1;
    message.channel.messages.fetch({ limit: amount }).then(async (messages) => {
      if (user) {
        ...
        try {
          ...
          return reply(`<a:aRedTick:584866618039861249> Couldn't delete messages because ${e}.`);
        }
        ...
        return reply(`<a:aGreenTick:584866617780076576> Successfully deleted **${messages.length}** of **${user.user.tag}**'s messages!`);
      }
      ...
      reply(`<a:aGreenTick:584866617780076576> Successfully deleted **${amount}** messsages!`);
    });
  }
}
