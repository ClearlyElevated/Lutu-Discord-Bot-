...
class Case extends Command {
  constructor (client) {
    super(client, {
      name: "case",
      description: "Approve/Deny Global Ban list Reports.",
      category: "Ban List",
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    if (!args[1]) return reply("You must pick an option, reject it or approve it.");
    ...
    if (args[1].toLowerCase() === "approve") {
      ...
      const approvedEmbed = new Discord.MessageEmbed()
        ...
        .setColor("#2ECC71")
        .setTimestamp();
      this.client.channels.get("590426129752391690").send(approvedEmbed);
      ...
      const rejectedEmbed = new Discord.MessageEmbed()
        ...
        .setColor("FF0000")
        .setTimestamp();
      this.client.channels.get("584320551204421642").send(rejectedEmbed);
