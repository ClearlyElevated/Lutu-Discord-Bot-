  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    Settings.findOne({
      ...
      if (args[0].toLowerCase() === "reset") {
        ...
        return reply("<a:aGreenTick:556121203136528388> Auto join role has been reset on this server.");
      }

      const role = message.mentions.roles.first() || message.guild.roles.get(args[0]);
      if (!role) return reply("<a:aRedTick:556121032507916290> You must specify a role to be marked as auto join role.");
      ...
      await settings.save().catch(e => this.client.logger.log(e));
      return reply(`<a:aGreenTick:556121203136528388> Set ${role} as the join auto role for this server.`);
    });
  }
}
