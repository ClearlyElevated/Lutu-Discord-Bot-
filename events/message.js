  ...
  async run (message) {
    ...
    if (message.content.match(mentionHelp)) {
      const helpEmbed = new Discord.MessageEmbed()
        .setTitle("Lutu - THE ONLY MOD BOT YOU WILL EVER NEED!")
        .setDescription(`
...
Dashboard: https://Lutu.cf
Invite Lutu: https://bots.discord.io/lutubot
Support Server: https://discord.io/underground_server

Basic Commands:
\`${guildSettings.prefix}help\` - Get a list of commands.
\`${guildSettings.prefix}info\` - Basic information about Lutu.

**__Owner: ClearlyElevated#0001__**
    ...
    if (cmd && usrs.includes(message.author.id)) return reply("Seems like you can't acces this. You've been banned by one of bot admins for more details regarding your case and appeal info, join our support server https://discord.io/underground_server");
    ...
    if (!cmd) return;
    if (level < 9 && this.client.cmdMaintenance === true) return reply("We are currently undergoing a maintenance we'll be back soon. In the meantime. Feel free to join the server & visit the dashboard! https://lutu.cf");
    if (cmd.conf.enabled === false) return reply("<a:aRedTick:584866618039861249> This command is currently globally disabled.");
    ...
    if (message.author.id !== "542058542853521461" && message.author.id !== "") {
      if (!timestamps.has(message.author.id)) {
        ...
        if (now < expirationTime) {
          ...
          return reply(`Slow it down dude. You have to wait ${timeLeft.toFixed(1)} seconds before using \`${cmd.help.name}\` again. In meanwhile you can visit our brand new dashboard at https://lutu.cf.`);
        }
    ...
    const noPermEmbed = new Discord.MessageEmbed()
      ...
      .setDescription(`
<a:aRedTick:584866618039861249> Forbidden! You do not have the required permissions to use \`${cmd.help.name}\`.
...
Pro Tip: We now have a dashboard - (MIGRATING HOSTS)
          `)
      .setTimestamp();
...
`;
    ...
    this.client.channels.get("616098222276083723").send(`**${cmd.help.name}** was used by \`${message.author.tag}\` (ID: ${message.author.id}) in guild \`${message.guild ? message.guild.name : "Direct Message"}\` (ID: ${message.guild ? message.guild.id : "000000000000000000"}), channel \`${message.channel.name}\` (ID: ${message.channel.id}).\nMessage content: \n\`\`\`${message.content}\`\`\``);
    try {
      ...
      reply(`Internal error occured!\nError Code: \`${errorCode}\`\nPlease report this error to the developers. Type \`${guildSettings.prefix}invite\` to get a link to the support server. Visit our dashboard at https://lutu.cf/.`);
      this.client.channels.get("584321456360390696").send(`An internal error occured while running \`${cmd.help.name}.js\`.\n\`\`\`xl\n${e}\`\`\``);
    }
  }
};
